...
```
Oak Shell v0.1 (type "help" for commands, "exit" to quit)
/> ls
/> cd /home
/home> pwd
/home
...
- **No pipes or redirections**: Commands like `ls | grep` are not supported
- **No wildcards**: Glob patterns like `*.txt` are not expanded
- **No quotes in arguments**: Quoted strings with spaces are not parsed correctly
...
- [ ] Add quoted argument parsing
- [ ] Add command history and editing (readline-like)
- [ ] Add tab completion
- [ ] Add alias support
- [ ] Add more commands (`grep`, `find`, `head`, `tail`, etc.)
